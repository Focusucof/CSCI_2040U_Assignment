...
export default function PlaylistCard({ playlist }: PlaylistCardProps) {
  return (
    <div className="group relative bg-[#181818] hover:bg-[#252525] rounded-xl overflow-hidden transition-all duration-300 cursor-pointer hover:-translate-y-1">
      ...
      {/* Info */}
      <div className="p-3">
        <h3 className="text-sm font-bold text-white truncate group-hover:text-purple-300 transition-colors">{playlist.title}</h3>
        <p className="text-xs text-zinc-400 line-clamp-2 mt-1 leading-relaxed group-hover:text-zinc-300 transition-colors">{playlist.description}</p>
