...
export default function AlbumPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(ALBUMS_API).then(res => res.json()),
      fetch(SONGS_API).then(res => res.json())
    ])
      .then(([albumsData, songsData]) => {
        const foundAlbum = albumsData.find((a: Album) => a.id === albumId);
  ...
  const albumTracks = allSongs.filter((track) => track.album === album?.title).map(normalizeTrackUrl);
      ...
      <main className="flex-1 overflow-y-auto pb-28">
        <div className="p-8">
          ...
          <div className="flex items-center gap-6 mb-8">
            <div className="relative w-48 h-48 shadow-xl">
              <Image
            ...
            <div>
              <p className="text-sm text-zinc-400 uppercase tracking-wider mb-1">Album</p>
              <h1 className="text-4xl font-bold text-white mb-2">{album.title}</h1>
              <p className="text-zinc-300">{album.artist}</p>
              <p className="text-zinc-500 text-sm mt-1">
                {album.year} · {albumTracks.length} song{albumTracks.length !== 1 ? 's' : ''}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 mb-6">
            <button
              onClick={() => { if (albumTracks.length > 0) { setQueue(albumTracks); onTrackSelect(albumTracks[0]); } }}
          ...
          <div className="space-y-1">
            ...
            {albumTracks.map((track, index) => (
              <div
                ...
                onClick={() => { setQueue(albumTracks); onTrackSelect(track); }}
                onContextMenu={(e) => {
                  e.preventDefault();
